...
dataset_out <- paste0(dataset, "_oracle")
file_name = sprintf("results_%s_setting%s_yalpha%i_talpha%i_iters%s_times%s_fullalphaset%s.RData", dataset_out, setting, Y_ALPHA, T_ALPHA, iters, times, FULL_ALPHA_SET)
...
# True DGP alpha and beta (fixed across all iterations and settings)
set.seed(0)
qa_true <- 20
p_true  <- 1000
...
for(iter  in 1:iters) {
  ...
  # Use true DGP directions for deconfounding (oracle)
  alpha_hat <- alpha_true
  beta_hat  <- beta_true
  alpha_hat_normalized <- alpha_true
  beta_hat_normalized  <- beta_true
